...
function ProjectsPageHTTP(sources) {
  const PEOPLE_URL = `${API_PATH}/projects/`;
  ...
  const request$ = sources.props$.flatMap(({timeRange}) => {
  ...
  const response$ = atomicResponse$
    ...
    .withLatestFrom(atomicResponse$, (projects, response) => {
      return {
        projects,
        progress: projects.length / response.body.count,
      };
    });
...
export default ProjectsPageHTTP;

  ...
  updateFiltering() {
    ...
    let filtered = this.state.people.filter((person) => {
      return (
        ...
        || (this.state.filters.tribe === 'Helsinki'
          && HELSINKI.indexOf(person.tribe.name) !== -1)
        || (
          this.state.filters.tribe === 'Germany'
          && GERMANY.indexOf(person.tribe.name) !== -1)
        );
    });

    filtered = filtered.filter((person) => {
      const lowerCaseSearch = this.state.filters.search.toLowerCase();
      return (
        this.state.filters.search === null
    ...
    filtered = filtered.filter((person) => {
      ...
      return this.state.filters.availability === null
        || man_days_available >= this.state.filters.availability;
    });
            ...
            <div className="filters">
              ...
              <div className="filtertools bottom-border-line">
                ...
                <div className="textfilter">
                  ...
                  <TextFilter
                    searchString={ this.state.filters.search }
                    onChange={this.filterByString.bind(this)}
                    />
                </div>
                <AvailabilityFilter
                  availabilityFilterValue={this.state.filters.availability}
                  onChange={this.filterByAvailability.bind(this)}
                  />
                <TimeRangeFilter />
              </div>
            </div>
